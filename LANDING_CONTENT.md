# BW Player — Landing Page Copy

Short, catchy phrases per section. Mirror in `app/content/landing.ts`.

---

## Meta / SEO

- **Title:** BW Player | Monochrome music, full color when it plays
- **Description:** Stream, mix, and queue from Last.fm, Audius, or Jamendo. A calm player where album art is the only color on screen.
- **OG Title:** BW Player — Listen in black and white
- **OG Description:** Discover moods. Shape Smart Mixes. Own your library.

---

## Nav

- **Wordmark:** BW Player
- **Link Features:** Features
- **Link Download:** Download
- **CTA:** Get the app

---

## Hero

- **Eyebrow:** Monochrome Studio
- **Headline:** Sound, stripped to the essential.
- **Subhead:** Stream. Mix. Queue. Let the artwork breathe.
- **Primary CTA:** Get the app
- **Secondary CTA:** See the features
- **Mockup label:** Discover — trending now, one tap away

---

## Platform strip

- **Chip 1:** Android
- **Chip 2:** iOS
- **Chip 3:** macOS

---

## Features (bento)

### Discover

- **Title:** Discover
- **Line:** Trending now. One tap to play.
- **Mockup:** Featured hero + mood row

### Smart Mix

- **Title:** Smart Mix
- **Line:** Describe the vibe. Hear the mix.
- **Mockup:** Live sliders + waveform

### Now Playing

- **Title:** Now Playing
- **Line:** Cover art or pulse. Your call.
- **Mockup:** Full-screen focus

### Library

- **Title:** Library
- **Line:** Playlists that live on your device.
- **Mockup:** Recents, favorites, yours

### Design

- **Title:** Monochrome Studio
- **Line:** UI recedes. Music takes the stage.
- **Mockup:** (text-only card)

---

## Curated Moods

- **Section title:** Curated Moods
- **Section line:** Tap a mood. Start the mix.

### Focus

- **Title:** Focus
- **Line:** Deep work & concentration

### Energy

- **Title:** Energy
- **Line:** High-tempo motivation

### Calm

- **Title:** Calm
- **Line:** Soft ambient unwind

---

## Music Sources

- **Section title:** Your catalog. Your pick.
- **Last.fm:** Charts and discovery from Last.fm
- **Audius:** Open catalog, curated flow
- **Jamendo:** Creative Commons, ready to stream

---

## Download CTA

- **Headline:** Ready when you are.
- **Subhead:** Android, iOS, and macOS.
- **Android CTA:** Download for Android
- **iOS CTA:** Download for iOS
- **macOS CTA:** Download for macOS

---

## Footer

- **Copyright:** © {year} BW Player
- **Tagline:** Monochrome music player.
